// ==========================================
// COLLECTIBLES — Orb spawning, collection, scoring
// ==========================================
...
export const updateCollectibles = (delta, scene, car, gameState) => {
    for (const orb of orbs) {
        ...
        const pulse = 0.6 + Math.sin(orb.userData.floatOffset * 2) * 0.2;
        orb.material.opacity = pulse;

        const dist = car.position.distanceTo(orb.position);
        if (dist < 3) collectOrb(orb, scene, gameState);
    }
};
