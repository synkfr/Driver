...
const getCollisionNormal = (pos) => {
    ...
    if (Math.abs(dx) < half && Math.abs(dz) < half) {
        ...
        return penX < penZ
            ? new THREE.Vector3(dx > 0 ? 1 : -1, 0, 0)
            : new THREE.Vector3(0, 0, dz > 0 ? 1 : -1);
    }
    return null;
};
...
export const updatePhysics = (delta, gameState, carParts, scene) => {
    ...
    if (keys.s) thrust -= phys.braking;
    if (keys.space) thrust = 0;
    ...
    if (absSpeed > gear.max * 0.93 && trans.current < trans.gears.length - 1) {
        trans.current++;
        trans.rpm = 4500;
    } else if (trans.current > 0 && absSpeed < trans.gears[trans.current - 1].max * 0.70) {
        trans.current--;
    }
    ...
    if (absSlip > phys.peakSlipAngle * 1.5) isDrifting = true;
    ...
    if (isDrifting && absSpeed > 25) {
        ...
        if (skidMarkTimer > 0.18) {
            ...
            addSkidMark(scene, car.position.x + rearOff.x + sideL.x, car.position.z + rearOff.z + sideL.z, heading);
            addSkidMark(scene, car.position.x + rearOff.x + sideR.x, car.position.z + rearOff.z + sideR.z, heading);
            skidMarkTimer = 0;
        }
    }
